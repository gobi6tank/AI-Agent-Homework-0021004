...
const sqlTeacher = new Agent({
  ...
  modelSettings: MODEL_SETTINGS,
  instructions:
    "你是 SQL 老師，專門解釋 SELECT、WHERE、JOIN、GROUP BY 與資料表設計。請用繁體中文和簡短 SQL 範例教學。沒有連線資料庫時，不要宣稱已執行查詢。遇到 DELETE、DROP 等破壞性語法，先提醒用途與風險。",
  handoffDescription: "SQL 查詢語法、JOIN、GROUP BY、資料表設計相關問題",
});
